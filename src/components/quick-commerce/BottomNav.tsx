      ...
      <div className="max-w-md mx-auto flex justify-around items-center py-2 px-4">
        {navItems.map((item) => (
          <button
            ...
            className={`relative flex flex-col items-center gap-1 py-2 px-6 rounded-2xl transition-all duration-200 ${
              active === item.id
                ? 'text-primary'
          ...
          >
            <div className="relative">
              <item.icon className={`w-6 h-6 transition-transform ${active === item.id ? 'scale-110' : ''}`} />
